<template>
    <div>
        <div v-if="loader" class="loader loader-admin" style="z-index: 9999">
        ...
        <div v-if="!errorsServer">
            ...
            <div class="loan-block1">
                ...
                <div class="loans-footer">
                  <div class="card-payment-checkbox">
                    <span>{{ $t('pay_saved_card') }}</span>
                    <switches v-model="cardPayment" :type-bold="true" ></switches>
                  </div>
                  <card-block v-show="cardPayment" v-model="selectedCard"></card-block>
                  <button
                    class="button-yellow button-yellow-loan"
                    v-bind:class="{disabled : payAll === 0}"
                    @click="payAllLoans" v-if="loans.length>0">
                    {{$t('pay_total')}} <span> {{nicePrice(payAll)}} ₸</span>
                  </button>
                </div>
            </div>
        </div>
      <payment-info
        :success="paymentModal.status"
...
import {en, kk, ru} from 'vuejs-datepicker/dist/locale'
import CardBlock from '../blocks/cardBlock'
import PaymentInfo from '../blocks/modal/PaymentInfo'
...
export default {
  ...
  methods: {
    ...
    payAllLoans () {
      this.loader = true
      if (this.cardPayment && this.selectedCard.value !== undefined && typeof this.selectedCard.value === 'number') {
        this.payWithCard()
      } else {
        this.payWithoutCard()
      }
    ...
    async payWithCard () {
      ...
      const requestObject = {
        ...
        order_id: orderId,
        card_id: this.selectedCard.value
      }

      window.location.replace(`${this.$axios.defaults.baseURL}/auth/card/payment?${querystring.stringify(requestObject)}`)
    },
    async payWithoutCard () {
      const orderId = await this.createOrder()
      if (!orderId) {
        this.loader = false
        return null
      }
      const { data } = await this.$axios.post('/auth/req_pay', { order_id: orderId })
      location.href = data.data.url
      this.urlAddress()
    },
    async createOrder () {
      const requestObject = {
        loans: await this.getOrderData(),
        place: 'ЛКСайт'
      }

      const { status, data } = await this.$axios.post('/auth/regular_pay_order', requestObject)
      ...
      return data.data[0].order
    },
    setServerError (data) {
      if (data.code !== 1) {
        this.errorsServer = data.error
        this.loader = false
        return true
      }
      return false
    ...
    getOrderData () {
      return this.loans
        ...
        .map(i => ({
          ...
          amount: i.default_pay,
          entered: i.payment_sum
        }))
    },
    bottomScroll () {
...
<style scoped>
    ...
    .loans-footer .card-payment-checkbox {
    ...
    .loans-footer .card-payment-checkbox .vue-switcher {
      margin: 0 0 0 16px;
    }

    .loans-footer .button-yellow {
      grid-area: button;
      margin-left: 0;
    }

    .loans-footer .card-block {
      grid-area: cards;
    }
