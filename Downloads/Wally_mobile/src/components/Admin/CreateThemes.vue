<template>
    <div class="createPlan padding-30">
        <div class="title"><router-link to="/admin/themes"><img :src="require('@/assets/img/icon/back.svg')" alt="" class="img"></router-link>
          {{ $t('new_theme') }}</div>
        <div class="bg-white createPlan-form">
            <div class="form">
                ...
                <div class="form-1">
                    ...
                    <select name="type" id="TypeID" v-model="TypeID">
                ...
                <Button :text="$t('create')" @click="createTheme" :loading="loading"/>
    ...
    getEduTypes () {
      this.$axios.get(this.$axios.baseURL + '/api/gettopicedutype')
        .then(response => {
          ...
          this.EduType = this.eduTypes[0].ID
        })
    },
    createTheme () {
      let formData = {
      ...
      this.loading = true
      this.$axios.post(this.$axios.baseURL + '/api/subject/newtopic', formData)
        .then(r => {
          if (!r.data.status) {
            this.loading = false
