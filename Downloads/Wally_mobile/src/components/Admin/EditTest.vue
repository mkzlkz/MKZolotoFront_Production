  ...
  <div class="createPlan padding-30">
    ...
    <div class="bg-white createPlan-form">
      <div class="form">
        ...
        <button class="button-blue" v-on:click="update">Изменить</button>
...
export default {
  ...
  methods: {
    getTest () {
      this.$axios.get(this.$axios.baseURL + '/api/exam/gettest/' + this.testId)
        .then(response => {
          ...
          this.PassMarks = test.PassMarks
          this.PlanID = test.PlanID
          this.TopicID = test.TopicID
          this.Type = !test.Type
    ...
    update () {
      ...
      this.$axios.put(this.$axios.baseURL + '/api/exam/updatetest/' + this.testId, formData)
        .then(resp => {
          if (resp.data.status) {
