<template>
    <div class="video-list" >
        <div v-for="item in videoList"
             ...
             v-bind:key="item.id"
             class="video-item">
            <video v-if="(getCurrentVideo().id!=item.id)" controls autoplay playsinline ref="videos" :height="cameraHeight" :muted="item.muted" :id="item.id" :dataType="(getCurrentVideo().id!=item.id) ? 'local':'remote'"></video>
        </div>
    </div>
</template>
